                  ...
                  <div id="printableArea" class="table-responsive">
                    ...
                    <table id="example" class="table table-striped table-bordered nowrap" style="width:100%">
                      <form action="" method="GET">
                        <caption>
                          <div>Laporan penjualan / pesanan dibatalkan <p style="float:right;">Kasir: <?= $session_nama; ?></p>
                          </div>
                        </caption>
                        <thead>
                          <tr>
                        ...
                        <tbody>
                          ...
                          if (isset($_POST['pencarian'])) {
                            ...
                            $tanggal_akhir = $_POST['ke'];
                            // tampilkan data yang sesuai dengan range tanggal yang dicari 
                            $data = mysqli_query($conn, "SELECT * FROM pesanan WHERE tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND status_kirim ='dibatalkan'");
                          } else {
                            // $data = mysqli_query($conn, "select * from checkout ORDER BY order_id ASC");
                            $data = mysqli_query($conn, "SELECT * FROM pesanan WHERE status_kirim ='dibatalkan' ORDER BY id_pesanan ASC");
                          }
                          $no = 1;
                          ...
                          ?>
                            <tr>
                              <td><?php echo $no++; ?></td>
                              <td>
                                <p class="card-text m-1"><?= $d['daftar_pesanan']; ?>
                              </td>
